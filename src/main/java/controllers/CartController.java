...
public class CartController {
    ...
    public void handleGetCart(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            ...
            HttpUtil.sendJson(resp, HttpServletResponse.SC_UNAUTHORIZED, e.getMessage());
        }
    }
    ...
    public void handleAddToCart(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            ...
            HttpUtil.sendJson(resp, HttpServletResponse.SC_OK, "Added to cart");
        } catch (Exception e) {
            HttpUtil.sendJson(resp, HttpServletResponse.SC_BAD_REQUEST, e.getMessage());
        }
    }

    public void handleUpdateQuantity(HttpServletRequest req, HttpServletResponse resp, Integer cartItemId) throws IOException {
        try {
            Integer userId = JwtUtil.getUserIdFromAuthorizationHeader(req.getHeader("Authorization"));
    ...
    public void handleRemoveItem(HttpServletRequest req, HttpServletResponse resp, Integer cartItemId) throws IOException {
        try {
            Integer userId = JwtUtil.getUserIdFromAuthorizationHeader(req.getHeader("Authorization"));
