 ...
 * CartController
 * <p>
 * Responsibilities: - Parse request (JWT auth header + JSON body) - Delegate
 * business logic to CartService - Return JSON responses with appropriate HTTP
 * status codes
 * <p>
 * Notes: - Currently all business errors are returned as 400 BAD_REQUEST. (If
 * you later want more RESTful behavior, stock conflict can be mapped to 409
 * CONFLICT.)
 */
public class CartController {
    ...
    /**
     * GET /cart - Extract userId from Authorization header (JWT) - Return all
     * cart items of current user
     */
    ...
    /**
     * POST /cart/add Body: { "productId": number, "quantity": number } - Add a
     * product to user's cart (create cart if not exists)
     */
    ...
    /**
     * PUT /cart/items/{cartItemId} Body: { "quantity": number } - Update
     * quantity of a specific cart item (owned by current user) - If quantity <=
     * 0, the item will be removed
     */
    ...
    /**
     * DELETE /cart/items/{cartItemId} - Remove one cart item (owned by current
     * user)
     */
    ...
    /**
     * DELETE /cart - Clear entire cart of current user (typically after
     * checkout succeeds)
     */
